...
(
  ...
  begin
    ...
    else if ( vc_trace.level == 2 )
      $sformat( str, "%s:%x", type_str, msg.addr );
    else if ( vc_trace.level == 3 ) begin
      if ( type_ == c_read )
        $sformat( str, "%s:%x:%x %s", type_str, msg.opaque, msg.addr, {8{" "}} );
      else
...
(
  ...
  begin
    ...
    if ( vc_trace.level == 1 )
      ...
      if ( type_ == c_read )
        $sformat( str, "%s:%x:%x %s", type_str, msg.opaque, msg.addr, {32{" "}} );
      else
        $sformat( str, "%s:%x:%x:%x", type_str, msg.opaque, msg.addr, msg.data );
    end
...
//------------------------------------------------------------------------
// Memory Response Struct: Using a packed struct to represent the message
//------------------------------------------------------------------------
...
(
  ...
  begin
    ...
    if ( (vc_trace.level == 1) || (vc_trace.level == 2) )
      $sformat( str, "%s", type_str );
    else if ( vc_trace.level == 3 ) begin
      if ( type_ == c_write || type_ == c_write_init )
        $sformat( str, "%s:%x %s", type_str, opaque, {8{" "}} );
      else
...
(
  ...
  begin
    ...
    if ( (vc_trace.level == 1) || (vc_trace.level == 2) )
      ...
      if ( type_ == c_write || type_ == c_write_init )
        $sformat( str, "%s:%x %s", type_str, opaque, {32{" "}} );
      else
        $sformat( str, "%s:%x:%x", type_str, opaque, data );
    end
